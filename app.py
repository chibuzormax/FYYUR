...
class Show(db.Model):
    ...
    def __repr__(self) -> str:
        return f"<Show id={self.id} artist_id={self.artist_id} start_time={self.start_time}>"
...
@app.route('/')
def index():
  venues = Venue.query.order_by(db.desc(Venue.created_on)).limit(10).all()
  artists = Artist.query.order_by(db.desc(Artist.created_on)).limit(10).all()
...
def search_venues():
  ...
  #   }]
  # }
  return render_template('pages/search_venues.html', results=response, search_query=request.form.get('search_query', ''))
  ...
  for artist in artists:
    ...
    for show in artist.shows:
      if show.start_time > datetime.now():
        upcoming_shows = upcoming_shows + 1 #modify
...
def show_artist(artist_id):
  ...
  for show in upcoming_shows:
    ...
    temp["venue_name"] = show.venues.name
    temp["venue_id"] = show.venues.id
    temp["venue_image_link"] = show.venues.venue_image_link
    temp["start_time"] = show.start_time.strftime("%m/%d/%Y, %H:%M:%S")
    temp_shows.append(temp)
